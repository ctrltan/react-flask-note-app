...
from note_app.helpers.auth_functions import email_encryption, token_creator, token_decoder, create_session, remove_session, is_valid_session
from flask_jwt_extended import decode_token, create_access_token
from datetime import timedelta


def test_email_encryption_success():
...
def test_email_encryption_incorrect_input_failure():
    test_email = 1
    
    with pytest.raises(Exception):
        email_encryption(test_email)

def test_token_creator_success(client):
...
def test_token_creator_incomplete_payload_failure(client):
    test_payload = {'username': 'janedoe'}

    with pytest.raises(Exception):
        token_creator(test_payload)

def test_token_decoder_success(client):
...
def test_token_decoder_success_with_expired_token(client):
    test_claims = {'user_id': '123', 'session_id': '456', 'username': 'janedoe'}
...
def test_token_decoder_invalid_token(client):
...
def test_decoder_empty_token(client):
