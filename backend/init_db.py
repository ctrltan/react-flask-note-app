...
cur.execute('''CREATE TABLE IF NOT EXISTS users (id serial PRIMARY KEY, username VARCHAR(15), password VARCHAR(50));''')
cur.execute('''CREATE TABLE IF NOT EXISTS notes (id serial PRIMARY KEY, title VARCHAR(50), contents VARCHAR(1000));''')
cur.execute('''ALTER TABLE notes ADD CONSTRAINT user_id FOREIGN KEY (id) REFERENCES users(id);''')
